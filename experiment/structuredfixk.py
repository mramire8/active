...
def main():
    ...
    for t in range(args.trials):
        ...
        if args.student in "struct":
            ...
            student.set_score_model(expert) # cheating version, using expert
            ...
            student = structured.AALStructured(model=clf, accuracy_model=None, budget=args.budget, seed=t, vcn=vct,
                                                    subpool=250, cost_model=cost_model)
            student.set_score_model(exp_clf) # cheating part, using expert
        ...
        while 0 < student.budget and len(pool.remaining) > step_size and iteration <= args.maxiter:
            ...
            if not bootstrapped:
                ...
                query_chosen, _ = student.pick_next(pool=pool, step_size=step_size)
