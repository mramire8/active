...
class LogisticRegressionAdaptiveV2(LogisticRegression):
    ...
    def fit(self, X, y):
        ...
        clf = GridSearchCV(self.clf, tuned_parameters, scoring=score, cv=kcv, n_jobs=5, verbose=0)
        clf.fit(X, y)
...
class LogisticRegressionAdaptive3(LogisticRegression):

    def __init__(self, penalty='l2', dual=False, tol=1e-4, C=1.0,
                 fit_intercept=True, intercept_scaling=1, class_weight=None,
                 random_state=None):

        super(LogisticRegressionAdaptive3,self).__init__(
            penalty=penalty, dual=dual, tol=tol, C=C,
            fit_intercept=fit_intercept, intercept_scaling=intercept_scaling,
            class_weight=class_weight, random_state=random_state)
    ...
    def fit(self, X, y):
        ...
        clf = GridSearchCV(self.clf, tuned_parameters, scoring=score, cv=kcv, refit=True)
        clf.fit(X, y)
